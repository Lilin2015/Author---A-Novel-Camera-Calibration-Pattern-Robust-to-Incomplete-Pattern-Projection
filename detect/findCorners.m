...
% filter image
img_corners_m1 = zeros(size(img,1),size(img,2));
img_corners_m2 = zeros(size(img,1),size(img,2));
...
for template_class=1:size(template_props,1)
  ...
  % update corner map
  img_corners_m1 = max(img_corners_m1,img_corners_1);
  img_corners_m2 = max(img_corners_m2,img_corners_2);
  
end

img_corners = max(img_corners_m1,img_corners_m2);
...
corners.value = zeros(size(corners.p,1),1);
for i = 1:1:size(corners.p,1)
    if img_corners_m1(round(corners.p(i,2)),round(corners.p(i,1)))>...
            img_corners_m2(round(corners.p(i,2)),round(corners.p(i,1)))
        corners.value(i,1) = 1;
    elseif img_corners_m1(round(corners.p(i,2)),round(corners.p(i,1)))<...
            img_corners_m2(round(corners.p(i,2)),round(corners.p(i,1)))
        corners.value(i,1) = 0;
    else
        corners.value(i,1) = -1;
